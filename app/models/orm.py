...
class WorkerHeartbeat(Base):
    """Proof that some worker is alive and polling.

    Distinct from a job's own heartbeat: that says a specific job is
    progressing, this says anyone is home at all. A queued job with no
    worker running is otherwise indistinguishable from one waiting its
    turn — see the schema comment in db/init_schema.sql."""
